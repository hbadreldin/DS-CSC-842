...
while ( $_ = shift( @ARGV ) ) {
   ...
   switch: {
      ...
      if ( ! defined( $IPAddress ) ) {
         $IPAddress = $_ ;
         $DEBUG && print( "IPAddress = $IPAddress\n" );
         last switch ;
      }
      &DisplayUsage() ;
   }
...
sub getL2Info {
	...
	else { die( "*** Error[show ip arp $IPAddress]: IP Not in ARP table: "); }
}
...
sub RouterIP {
	...
	my @CoreOutput = $SSHtoCore->capture( "show ip route $IPAddress" ) or
